...
export default function ApplicationsCard({ apps, loading, updateStatus, removeApp }) {
  return (
      ...
      {loading ? null : apps.length === 0 ? (
        ...
        <div className="db-app-list">
          <div className="db-app-row db-app-header">
            ...
            <span className="db-app-del" />
          </div>

          {apps.map(({ id, company, role, status, date }) => (
            <div key={id} className="db-app-row">
              ...
              <button className="db-app-del" onClick={() => removeApp(id)}>×</button>
            </div>
          ))}
        </div>
      )}
